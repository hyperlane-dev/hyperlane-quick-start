...
/// 计算文件统计信息
pub async fn calculate_directory_stats(
    config: &ResourceConfig,
) -> Result<FileStats, StaticFileError> {
...
) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<(), StaticFileError>> + 'a>> {
    Box::pin(async move {
        use tokio::fs;
