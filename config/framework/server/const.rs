use super::*;

pub const SERVER_PORT: usize = 60007;
pub const SERVER_HOST: &str = "0.0.0.0";
pub const SERVER_WEB_SOCKET_BUFFER_SIZE: usize = 4096;
pub const SERVER_HTTP_LINE_BUFFER_SIZE: usize = 4096;
pub const SERVER_LOG_SIZE: usize = 100_024_000;
pub const SERVER_LOG_DIR: &str = "/home/group-chat/logs";
pub const SERVER_INNER_PRINT: bool = true;
pub const SERVER_INNER_LOG: bool = true;
pub const SERVER_NODELAY: bool = true;
pub const SERVER_LINGER: OptionDuration = None;
pub const SERVER_TTI: u32 = 128;
