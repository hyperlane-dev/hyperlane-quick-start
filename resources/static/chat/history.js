...
const ChatHistory = {
    ...
    loadHistory: async function() {
        ...
        try {
            const response = await fetch(
                `/api/chat/history?session_id=${encodeURIComponent(this.sessionId)}&offset=${this.offset}&limit=${this.limit}`
            );
    ...
    prependMessages: function(messages) {
        ...
        // Messages come in DESC order (newest first), so reverse them
    ...
    createMessageElement: function(msg) {
        ...
        messageDiv.className = `message ${isSelf ? 'self-message' : 'other-message'}`;
        ...
        const processedContent = this.processMessageContent(msg.content, msg.message_type);
        
        messageDiv.innerHTML = `
            ${!isSelf ? `<div class="message-avatar" style="background-color: ${avatarColor}">${avatarText}</div>` : ''}
            <div class="message-info ${isSelf ? 'self' : ''}">
                ${!isSelf ? `<div class="message-header"><span class="message-name">${msg.sender_name}</span></div>` : ''}
